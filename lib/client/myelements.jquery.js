 ...
 */
$myelementsHost = window.$myelementsHost || "";
...
jQuery.extend($myelements, {
  ...
  initReactOnMessage: function($el, scope) {
    $myelements.socket.on("message", function(message) {
      $myelements.debug("myelements message '%s' caught by data-react-on-message", data.reactOnMessage);
...
MyElement = jQuery.widget("myelements.myelement", {
  _create: function() {

    var dataAttributeOptions = this.element.data();
    this.options = jQuery.extend(dataAttributeOptions, this.options)
  },
  _init: function() {
...
jQuery.widget("meyelements.input", {
  _init: function() {
